...
static class Camera
{
    public static Vector2 CameraLoc(Viewport vp, PlayerCharacter character, DungeonMap map, Vector2 _cameraPos,
        GameTime gameTime, float worldScale, float CAMERA_SMOOTH_SPEED, bool snap, float UiTopPaddingPx)
    {
        float viewWorldWidth = vp.Width / worldScale;
        float viewWorldHeight = vp.Height / worldScale;
        float uiWorldPadY = UiTopPaddingPx / worldScale;
        ...
        var _cameraTarget = playerCenter - new Vector2(
            viewWorldWidth * 0.5f,
            (viewWorldHeight - uiWorldPadY) * 0.5f);
        ...
        float maxY = MathF.Max(0f, mapHeight - (viewWorldHeight - uiWorldPadY));
